...
import { cn, feedbackTypeMap } from "@/utils/format";
...
const labelToTypeKey: Record<string, string> = { "课程内容": "course_content", "作业批改": "homework", "老师服务": "teacher_service", "平台体验": "platform", "其他建议": "other" };
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => {
        ...
        return (
          <Star key={i} className={cn(filled || isHalf ? "fill-current text-ember-500" : "text-ink-200")} style={{ width: size, height: size }} />
        );
      })}
    </div>
  );
...
const TrendChip = ({ up, value, good }: { up?: "up" | "down" | "flat"; value: string; good?: boolean }) => {
  ...
  const colorClass = up === "flat" ? "text-ink-500 bg-ink-100" : (good ?? up === "up") ? "text-moss-600 bg-moss-50" : "text-ember-500 bg-ember-50";
  return (<span className={cn("chip gap-1", colorClass)}><Icon className="w-3 h-3" />{value}</span>);
};

type DropdownProps = { label: string; value: string; options: string[]; onChange: (v: string) => void; icon?: React.ComponentType<{ className?: string }> };
...
type OverviewCardProps = { label: string; value: string | number; icon: React.ComponentType<{ className?: string }>; iconBg: string; iconColor: string; chip?: React.ReactNode; subValue?: React.ReactNode };
...
export default function Analytics() {
  ...
  const computed = useMemo(() => {
    ...
    const avgSatisfaction = total > 0 ? Math.round((filtered.reduce((s, f) => s + f.satisfaction, 0) / total) * 10) / 10 : 0;
    ...
    filtered.forEach((f) => {
      ...
      courseMap[f.courseName].count++;
      courseMap[f.courseName].sat += f.satisfaction;
    });
    const topCourses = Object.entries(courseMap)
      .map(([name, v]) => ({ name, count: v.count, avgSat: Math.round((v.sat / v.count) * 10) / 10 }))
      .sort((a, b) => b.count - a.count)
      .slice(0, 5);

    const teacherMap: Record<string, { count: number; sat: number }> = {};
    filtered.forEach((f) => {
      if (!f.teacherName) return;
      if (!teacherMap[f.teacherName]) teacherMap[f.teacherName] = { count: 0, sat: 0 };
      teacherMap[f.teacherName].count++;
      teacherMap[f.teacherName].sat += f.satisfaction;
    });
    ...
    const trends = analytics.trends.map((t) => ({
      ...t,
      count: Math.round(t.count * (total / Math.max(analytics.total, 1))),
      satisfaction: total > 0 ? Math.round(avgSatisfaction * 10) / 10 : t.satisfaction,
    }));

    return { total, processing, closed, avgSatisfaction, byType, bySatisfaction, handleDuration, topCourses, topTeachers, trends };
  }, [filtered, analytics]);
  ...
  const wordColor = (weight: number) => { if (weight >= 70) return "#F56E17"; if (weight >= 40) return "#2F806B"; return "#5A6B6E"; };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <OverviewCard label="总反馈量" value={computed.total} icon={Inbox} iconBg="bg-moss-50" iconColor="text-moss-600" chip={<TrendChip up="up" value="+18% 周趋势" good />} />
        <OverviewCard label="处理中" value={computed.processing} icon={Gauge} iconBg="bg-sky2-50" iconColor="text-sky2-600" chip={<span className="chip bg-sky2-50 text-sky2-600">进行中</span>} />
        <OverviewCard label="已关闭" value={computed.closed} icon={CheckCircle} iconBg="bg-cream-200" iconColor="text-ink-700" subValue={<span className="chip bg-moss-100 text-moss-700 mb-1">关闭率 {closeRate}%</span>} />
        <OverviewCard label="平均处理时长" value={`${analytics.avgHandleHours}h`} icon={Clock} iconBg="bg-cream-200" iconColor="text-ink-700" chip={<TrendChip up="down" value="-2.3h 周同比" good />} />
        <OverviewCard label="平均满意度" value={computed.avgSatisfaction} icon={Star} iconBg="bg-ember-50" iconColor="text-ember-500" subValue={<div className="mb-1"><Stars rating={computed.avgSatisfaction} /></div>} />
      </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={computed.byType} dataKey="count" nameKey="type" cx="50%" cy="50%" innerRadius={55} outerRadius={100} paddingAngle={3}
                    ...
                    {computed.byType.map((_, idx) => (<Cell key={idx} fill={PIE_COLORS[idx % PIE_COLORS.length]} stroke="#FAF8F5" strokeWidth={2} />))}
                  </Pie>
